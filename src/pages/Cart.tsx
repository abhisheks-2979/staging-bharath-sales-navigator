...
React.useEffect(() => {
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      // Create invoice and send via WhatsApp automatically
      ...
      try {
        ...
        if (companyData && validRetailerId) {
          ...
          if (!invoiceError && invoice) {
            ...
            const invoiceItems = cartItems.map(item => ({
              ...
              taxable_amount: computeItemTotal(item),
              gst_rate: 9, // 9% CGST + 9% SGST = 18% total
              cgst_amount: computeItemTotal(item) * 0.09,
              sgst_amount: computeItemTotal(item) * 0.09,
              total_amount: computeItemTotal(item) * 1.18
            }));
            ...
            if (retailerPhone) {
              console.log('Sending invoice to WhatsApp:', retailerPhone);
              const { data: whatsappData, error: whatsappError } = await supabase.functions.invoke('send-invoice-whatsapp', {
                body: { 
              ...
              if (whatsappError) {
                ...
                console.log('Invoice sent via WhatsApp successfully');
        ...
        // Don't block order submission if invoice fails
      }

      const orderType = isCreditOrder ? "Credit Order" : "Order";
      const whatsappMsg = invoiceSentViaWhatsApp ? " Invoice sent via WhatsApp." : "";
      toast({
        title: `${orderType} Submitted`,
        description: isCreditOrder 
          ? `${orderType} for ${retailerName} submitted with ₹${creditPending.toLocaleString()} pending!${whatsappMsg}`
          : `Order for ${retailerName} submitted successfully!${whatsappMsg}`
      });
